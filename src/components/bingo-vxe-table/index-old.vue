<template>
    <div class="bingo-vxe-table" ref="table-list" v-resize="handleResize" :class="{'bingo-vxe-table-full-screen': isFullscreen}">
        <!--toolbar-new区域-->
        <bingo-toolbar-new ref="toolbar" v-if="showToolbar" v-bind="$attrs" v-on="$listeners" @on-full-screen="handleFullScreen">
            <!--左侧标题-->
            <template #h3>
                <slot name='h3'></slot>
            </template>
        ...
        <template>
            <div v-if="!showOtherView" class="bingo-vxe-table-body" :class="showPageBottom ? ' show-page-' + sizeVal : ''" :style="getBodyHeight">
                ...
                <vxe-pager
                    ...
                    <template #right>
                        <bmsa-layout class="bingo-vxe-table-pager-right"
                                     v-if="layoutName"
