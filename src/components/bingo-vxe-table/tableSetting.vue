                    ...
                    <Card class="layout-card-class" :bordered="false" dis-hover>
                        ...
                        <div class="layout-show-body">
                            <div class="flex1">
                                ...
                                <p>
                                    <Radio-group v-model="selectLayout.layoutSize" type="button" size="small" @on-change="handleSizeChange">
                                ...
                                <p>
                                    <Radio-group v-model="selectLayout.page" @on-change="handlePageChange" type="button" size="small">
                                        ...
                                        <Radio label="all">{{$t('components.table.all')}}</Radio>
                                    </Radio-group>
                                </p>
...
<script>
    ...
    // 新增接口
    const insertTablelayout = (data) => service({url: '/api/v1/masterdata/layout/layout-management', method: 'post', data});
    // 修改接口
    const updateTablelayout = (data) => service({url: '/api/v1/masterdata/layout/layout-management', method: 'put', data});
    // 删除接口
    const deleteTablelayout = (params) => service({url: '/api/v1/masterdata/layout/layout-management/'+params, method: 'delete'});
    // 查询全部接口
    const queryTablelayout = (params) => service({url: '/api/v1/masterdata/layout/layout-management/list', method: 'get', params});
    export default {
        name: 'tableSetting',
        props: {
            // 表格列，所有列，基础列
            ...
            return {
                ...
                formRules: { // 新建布局的规则
                    projectName: [
                        ...
                        { min: 3, max: 10, message: this.$t('components.table.length') + ' 3 ' + this.$t('components.table.to') + ' 10 ' + this.$t('components.table.numStr')},
                        {
                            validator ({ itemValue }) {
                                let flag = false
                ...
                sortable2: {}, // 列拖拽
                sizeGlobal: 'media', // 全局布局
            }
        },
        computed: {
            // 当前用户
            $user () {
                return {userId:this.$store.state.admin.user.info.userId};
            },
            getColumnSelect: { // 获取选中的列集合
                get: function () {
                    let selectArr = []
                    if (!this.selectLayout.layoutColumn) {
                        return selectArr
                    }
                    JSON.parse(this.selectLayout.layoutColumn).forEach(item => {
        ...
        methods: {
            ...
            handleTab (val) {
                ...
                if(val == 2){
                    this.selectLayout = {
                        ...
                        layoutSize: this.sizeGlobal,
                        page: 40,
                        froze: 1,
                        layoutColumn: JSON.stringify(this.column)
                    }
                }else{
                    this.handleSaveDefault();
                }
            ...
            handleReloadLayout (val) {
                ...
                if (this.isChangeDefault) {
                    ...
                    updateTablelayout(Object.assign(this.selectLayout, {isDefault:1})).then(res=>{
                        that.handleGetLayoutData() // 走布局方法，获取数据，渲染表格
                    }).catch(err=>{
                    })
                } else {
                    that.handleGetLayoutData() // 走布局方法，获取数据，渲染表格
            ...
            handleSubmit () {
                this.$refs['bmsaLayoutForm'].validate((valid) => {
                    if (!valid) {
                        ...
                        this.activeTab = '1'
                        let size = that.selectLayout.layoutSize || that.sizeGlobal;
                        let newLayout = [
                            that.layoutName,
                            ...
                            that.selectLayout.layoutColumn,
                            size,
                            that.$Date().format('YYYY-MM-DD')
                        ]
                        insertTablelayout(Object.assign(that.$user, {
                            layout:newLayout[0],
                            name:newLayout[1],
                            isDefault:newLayout[2],
                            page:newLayout[3],
                            froze:newLayout[4],
                            layoutColumn:newLayout[5],
                            layoutSize:newLayout[6],
                            createDate:newLayout[7],
                        }) ).then(res=>{
                            if(res.data){ that.$BMessage.success(res.data.msg)
                            }else{that.$BMessage.warning(res.data.msg);}
                        }).catch(err=>{
                            that.$BMessage.warning(err);
                        })
            ...
            handleDeleteLayout (layout, index) {
                ...
                this.$BAlert.info({
                    ...
                    onOk: (e) => {
                        ...
                        // 删除后定位到默认布局
                        deleteTablelayout(layout.id).then(res=>{
                            that.handleReloadLayout('默认布局')
                            e.show = false // 关闭弹窗

                            that.$BMessage.success('删除成功！')
                        }).catch(err=>{
                        });
                    }
                });
            ...
            handleCheckAll () {
                ...
                let that = this
                let columnFields = JSON.parse(this.selectLayout.layoutColumn)
                columnFields.forEach(item => {
                    if (item.field && item.type != 'group') {
                        item.visible = flag
                        if (item.children) { // 此处有children代表可能有下拉过滤
                            this.handleChangeVisible(item.children, flag)
                        }
                    }
                })
                this.selectLayout.layoutColumn = JSON.stringify(columnFields)
                if (!(that.selectLayout.name == '默认布局')) { // 默认布局不可以修改
                    // 存储到表中
                    updateTablelayout(Object.assign(that.selectLayout, {layoutColumn:that.selectLayout.layoutColumn})).then(res=>{
                    }).catch(err=>{
                    })
                }
            ...
            handleSizeChange (val) {
                ...
                if (!(that.selectLayout.name == '默认布局')) { // 默认布局不可以修改
                    // 存储到表中
                    updateTablelayout(Object.assign(that.selectLayout, {layoutSize:that.selectLayout.layoutSize})).then(res=>{
                        that.handleGetLayout() // 触发表格渲染
                    }).catch(err=>{
                    })
                } else {
            ...
            handlePageChange (val) {
                ...
                if (!(that.selectLayout.name == '默认布局')) { // 默认布局不可以修改
                    // 存储到表中
                    updateTablelayout(Object.assign(that.selectLayout, {page:that.selectLayout.page})).then(res=>{
                    }).catch(err=>{
                    })
                }
                this.layoutArr.forEach((item, index) => {
            ...
            handleFrozeChange (val) {
                ...
                if (!(that.selectLayout.name == '默认布局')) { // 默认布局不可以修改
                    // 存储到表中
                    updateTablelayout(Object.assign(that.selectLayout, {froze:that.selectLayout.froze})).then(res=>{
                    }).catch(err=>{
                    })
                }
                this.layoutArr.forEach((item, index) => {
            ...
            // 修改子列隐藏/显示
            handleChangeVisible (children, flag) {
            ...
            handleColumnChange (data) {
                ...
                columnFields.forEach(item => {
                    if (item.field && item.type != 'group') {
                        item.visible = false
                        if (item.children) { // 此处有children代表可能有下拉过滤
                            this.handleChangeVisible(item.children, false)
                        }
                        data.forEach(item2 => {
                            if (item.field == item2) {
                                item.visible = true
                                if (item.children) {
                                    this.handleChangeVisible(item.children, true)
                                }
                            }
                        })
                    }
            ...
            handleCheckDrag () {
                ...
                this.sortable2 = Sortable.create(this.$refs.checkGroup.$el, {
                    ...
                    onEnd: ({ item, newIndex, oldIndex }) => {
                        let addColumn = JSON.parse(this.selectLayout.layoutColumn)
                        // 去掉序号和布局列和分组列
                        let indexArr = []
                        let columnArr = []
                        ...
                        this.selectLayout.layoutColumn = JSON.stringify(addColumn)
                        if (!(that.selectLayout.name == '默认布局')) { // 默认布局不可以修改
                            // 存储到表中
                            updateTablelayout(Object.assign(that.selectLayout, {layoutColumn:that.selectLayout.layoutColumn})).then(res=>{
                            }).catch(err=>{
            ...
            handleSaveDefault () {
                let that = this
                queryTablelayout({layout:that.layoutName, userId:that.$user.userId}).then(res=>{
                    // res.data.data.length
                    // 去表里查询此条数据是否已存在
                    let len = res.data.data ?res.data.data.length:0;
                    that.layoutArr = []
                    if (len && len > 0) {
                        let selectLayout = {} // 获取默认选中的布局
                        for (let i = 0; i < len; i++) { // 所有的布局
                            let layout = res.data.data[i]
                            let baseColumn = JSON.parse(JSON.stringify(that.column)) // 全局列
                            let layoutColumn = JSON.parse(layout.layoutColumn) // 布局存储的列
                            let addColumn = [] // 用于记录开发人员新增的列
                            let newColumn = [] // 全局列和布局列最终合并完的列
                            if (layout.name == '默认布局') { // 默认布局直接被全局列覆盖
                                newColumn = baseColumn
                            } else { // 自定义布局根据全局列来确定哪些列新增或者删除
                                ...
                                baseColumn.forEach((item, index) => {
                                    ...
                                    for (let j = 0; j < layoutColumn.length; j++) {
                                        ...
                                        if (item.field && item.type != 'group' && item.field == item2.field) {
                                            ...
                                            if (item.children) { // 此处有children代表可能有下拉过滤
                                                this.handleChangeVisible(item.children, item2.visible)
                                            }
                                            isNew = false
                                            break
                                        }
                            ...
                            if (layout.isDefault) { // 找到当前选中的布局
                                selectLayout = {
                                    ...
                                    layoutSize: layout.layoutSize || that.sizeGlobal,
                                    page: layout.page,
                                    froze: layout.froze,
                                    name: layout.name,
                                    isDefault:1,
                                    layoutColumn: JSON.stringify(newColumn), // 这里取全局列和布局存储列构造完的结构
                                }
                            }
                            ...
                            row.layoutSize =  row.layoutSize || that.sizeGlobal  // 有布局取当前布局，没有取全局布局
                            row.layoutColumn = saveColumn
                            that.layoutArr.push(row) // 所有布局的集合
                            updateTablelayout(Object.assign(row, {layoutColumn:saveColumn})).then(res=>{
                            }).catch(err=>{
                            })
                        }
                        that.selectLayout = selectLayout  // 当前选中的布局
                    } else {
                        let size = that.size || that.sizeGlobal
                        // 存储默认布局
                        let defaultLayout = [
                            that.layoutName,
                            '默认布局',
                        ...
                        insertTablelayout(Object.assign(that.$user, {
                            ...
                            name:defaultLayout[1],
                            isDefault:defaultLayout[2],
                            page:defaultLayout[3],
                            froze:defaultLayout[4],
                            layoutColumn:defaultLayout[5],
                            layoutSize:defaultLayout[6],
                            createDate:defaultLayout[7],
                        }) ).then(res=>{
                            if(res.data){ 
                            }else{that.$BMessage.warning(res.data.msg);}
                        }).catch(err=>{
                        ...
                        that.selectLayout = {layout: that.layoutName, layoutSize: size, page: 40, froze: 1, name: '默认布局', layoutColumn: JSON.stringify(that.column)} // 当前选中的布局
                        that.layoutArr.push({
                            ...that.selectLayout,
                            isDefault: 1
                        })
            ...
            handleGetLayout () {
                var ags = JSON.parse(JSON.stringify(this.selectLayout));
                ags.column = this.selectLayout.layoutColumn;
                ags.size = this.selectLayout.layoutSize;
                this.$emit('getLayout', ags )
            },
        },
        mounted () {
            // 列拖拽
