        ...
        <Dropdown transfer ref="contentMenu" style="display: none" trigger="click" transfer-class-name="tree-drop-down"
            ...
            <DropdownMenu slot="list">
                ...
                <DropdownItem name="add" v-if="!disabledAdd">
                    <Icon custom="iconfont iconxinjian" style="margin-right: 10px"></Icon>{{this.addName}}
                </DropdownItem>
                <!-- by zhengdj -->
                <slot name="centerItem" :nodeInfo="nodeInfo"></slot>
...
<script>
    export default {
        ...
        props: {
            ...
            addNodeBefore:{ // 添加前的方法，回调  by zhengdj
                type: Function,
        ...
        methods: {
            ...
            renderContent (h, { root, node, data }) {
                return h('span', {
                    ...
                    on: {
                            ...
                            if (!data.disabled) { // 控制节点是否可以选择
                                ...
                                _this.timeId = setTimeout(function () {
                                    if (_this.multiple) {
                                        ...
                                        // if(_this.$refs.tree.getSelectedNodes()==0){
                                        //     data.selected = true
                                        // } 控制当前选中的节点不可以取消选中  是不是应该在外面判断 by zhengdj
                                        if(!data.editState){ // 输入框的内容点击不触发 by zhengdj
                                            _this.$emit('on-change', _this.$refs.tree.getSelectedNodes(), data) // 换个触发事件的名字，要不走两遍 by zhengdj
                                        }
                        ...
                        contextmenu: (e) => {
                            // 当有默认按钮显示或者有自定义按钮显示时
                            if (!(this.disabledAll || data.disabledAll)) {
                    ...
                    h('div', {
                        ...
                        h('i', {
                            class:`ivu-icon ${data.prevIcon || this.prevIcon}`,
                        }),  // 图标  by zhengdj
                        h(`${data.editState ? '' : 'span'}`, data[this.rowName]),
                        h(`${data.editState ? 'input' : ''}`, {
                            class: 'ivu-input ivu-input-small ivu-input-edit',
                            attrs: {
                                value: '', // 解决光标问题   by zhengdj
                                placeholder: data[this.rowName] && data[this.rowName].indexOf('新建节点') >= 0 ? data[this.rowName] : '',
                                autofocus: true,
            ...
            editTree (data) {
                ...
                this.$nextTick(() => { // 编辑框出现后自动聚焦
                    this.$refs.tree.$el.getElementsByClassName('ivu-input-edit')[0].value = data[this.rowName]; // 解决光标问题   by zhengdj
            ...
            addNode () {
                event.stopPropagation()
                let _this = this
                if (this.addNodeBefore && (typeof this.addNodeBefore === 'function')) {   // 加一个添加节点之前的方法  by zhengdj
                    _this.addNodeBefore.apply(this, [this.$attrs.data, this.node, ()=> {
